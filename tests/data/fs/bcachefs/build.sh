...
truncate -s 32M /tmp/output.bcachefs
bcachefs format /tmp/output.bcachefs

"$RUN_SCRIPT" x86_64 "$KERNEL" "$BOOT_IMG" \
    -i "$SRC_IMG" -i /tmp/output.bcachefs -m duplicate

mkdir -p "$(dirname "$BUILD/$OUTPUT_PATH")"
cp /tmp/output.bcachefs "$BUILD/$OUTPUT_PATH"
