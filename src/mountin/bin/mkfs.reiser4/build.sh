...
find . -name mkfs.reiser4 -type f -executable | head -1 | xargs cp -v -t /host/build/bin/${MOUNTIN_TARGET_ARCH}-linux-gnu/
# busy tool for userspace file manipulation (create/cp/ls without mounting)
cp -v $(find . -name busy -type f -executable | head -1) /host/build/bin/${MOUNTIN_TARGET_ARCH}-linux-gnu/reiser4-busy
