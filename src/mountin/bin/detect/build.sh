...
for arch in x86_64 aarch64; do
    if want "bin/${arch}-linux-musl/detect"; then
        $ZIG cc -target ${arch}-linux-musl \
            -O2 -static \
            -I${INCLUDE} \
            ${SRCS} ${LIB}/${arch}-linux-musl/libmountin.a -lunwind \
            -o detect
...
for arch in x86_64 aarch64; do
    if want "bin/${arch}-linux-gnu/detect"; then
        $ZIG cc -target ${arch}-linux-gnu \
            -O2 \
            -I${INCLUDE} \
            ${SRCS} ${LIB}/${arch}-linux-gnu/libmountin.a -lunwind \
            -o detect
...
if want "bin/x86_64-windows-gnu/detect"; then
    $ZIG cc -target x86_64-windows-gnu \
        -O2 \
        -I${INCLUDE} \
        ${SRCS} ${LIB}/x86_64-windows-gnu/mountin.lib -lunwind \
        -o detect.exe
...
for arch in x86_64 aarch64; do
    if want "bin/${arch}-darwin/detect"; then
        $ZIG cc -target ${arch}-macos \
            -O2 \
            -I${INCLUDE} \
            ${SRCS} ${LIB}/${arch}-darwin/libmountin.a -lunwind \
            -o detect
