...
from its input hash. Injected `MOUNTIN_BUILD_*` variables and
`MOUNTIN_CACHE_DIR` are likewise operational. Injected `MOUNTIN_TARGET_*`
variables are semantic and are hashed. Catalogue-only context such as
`MOUNTIN_RELEASE_REF` and `MOUNTIN_SOURCE_KIND` is available for resolution but
is not exported to every provider.
