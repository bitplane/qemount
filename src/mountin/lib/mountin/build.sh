...
for arch in x86_64 aarch64; do
    if want "lib/${arch}-linux-musl/"; then
        ...
        # are still emitted.
        cargo zigbuild --quiet --release --target ${arch}-unknown-linux-musl -j $BUILD_JOBS
...
for arch in x86_64 aarch64; do
    if want "lib/${arch}-linux-gnu/"; then
        cargo zigbuild --release --target ${arch}-unknown-linux-gnu.2.17 -j $BUILD_JOBS
...
if want "lib/x86_64-windows-gnu/"; then
    cargo zigbuild --release --target x86_64-pc-windows-gnu -j $BUILD_JOBS
...
for arch in x86_64 aarch64; do
    if want "lib/${arch}-darwin/"; then
        cargo zigbuild --release --target ${arch}-apple-darwin -j $BUILD_JOBS
...
if want "lib/wasm32-wasi/"; then
    cargo zigbuild --release --target wasm32-wasip1 -j $BUILD_JOBS
